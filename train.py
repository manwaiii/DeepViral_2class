...
# training
filename_codetrfw = [ x for x in os.listdir(inDirTr) if 'codefw.npy' in x and 'bacterial' in x and contigLengthk + 'k' in x ][0]
print("data for training " + filename_codetrfw)
bac_trfw = np.load(os.path.join(inDirTr, filename_codetrfw))
bac_trbw = np.load(os.path.join(inDirTr, filename_codetrfw.replace('fw', 'bw')))
# validation
filename_codevalfw = [ x for x in os.listdir(inDirVal) if 'codefw.npy' in x and 'bacterial' in x and contigLengthk + 'k' in x ][0]
print("data for validation " + filename_codevalfw)
bac_valfw = np.load(os.path.join(inDirVal, filename_codevalfw))
bac_valbw = np.load(os.path.join(inDirVal, filename_codevalfw.replace('fw', 'bw')))


######## combine RNA, DNA and bacterial data ##########
print("...combining all training data...")
# combine x and y training data
Y_tr = np.concatenate((np.repeat(0, rna_trfw.shape[0]), np.repeat(1, dna_trfw.shape[0]), np.repeat(2, bac_trfw.shape[0])))
X_trfw = np.concatenate((rna_trfw, dna_trfw, bac_trfw), axis=0)
del rna_trfw, dna_trfw, bac_trfw
X_trbw = np.concatenate((rna_trbw, dna_trbw, bac_trbw), axis=0)
del rna_trbw, dna_trbw, bac_valfw
...
### validation V+B
Y_val = np.concatenate((np.repeat(0, rna_alfw.shape[0]), np.repeat(1, dna_valfw.shape[0]), np.repeat(2, bac_valfw.shape[0])))
X_valfw = np.concatenate((rna_valfw, dna_valfw, bac_valfw), axis=0)
del rna_valfw, dna_valfw, bac_valfw
X_valbw = np.concatenate((rna_valbw, dna_valbw, bac_valbw), axis=0)
del rna_valbw, dna_valbw, bac_valfw
