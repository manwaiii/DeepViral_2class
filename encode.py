...
# date              :20180807
# version           :1.0
# usage             :python encode.py -i ./training_data/tr/RNA_tr.fasta -l 1000
...
#            print("seqCatLen="+str(len(seqCat)))
            ...
            while posEnd <= len(seqCat) :
                contigName = NCBIName+"#"+str(contigLengthk)+"k#"+head.split('/')[-1]+"#"+str(pos)+"#"+str(posEnd)
...
#                    print("remove seq for >30% Ns {}".format(seq))
                ...
                if len(seqname) > 0 and len(seqname) % 4000000 == 0 :
                    ...
                    fileCount += 1
                    codeFileNamefw = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount)+"_seq"+str(len(code))+"_codefw.npy"
                    codeFileNamebw = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount)+"_seq"+str(len(codeR))+"_codebw.npy"
                    nameFileName = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount)+"_seq"+str(int(len(seqname)/2))+".fasta"     
...
if len(code) > 0 :
    codeFileNamefw = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount+1)+"_seq"+str(len(code))+"_codefw.npy"
    codeFileNamebw = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount+1)+"_seq"+str(len(codeR))+"_codebw.npy"
    nameFileName = NCBIName+"#"+str(contigLengthk)+"k_num"+str(fileCount+1)+"_seq"+str(int(len(seqname)/2))+".fasta"
    print("encoded sequences are saved in {}".format(codeFileNamefw))
    np.save( os.path.join(outDir, codeFileNamefw), np.array(code) )
    np.save( os.path.join(outDir, codeFileNamebw), np.array(codeR) )
    seqnameF = open(os.path.join(outDir, nameFileName), "w")
