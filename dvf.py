...
def pred(ID) :
    ...
    if seqL < 1000 :
      model = modDict['0.5']
      null = nullDict['0.5']
      print("Model with 0.5k is used")
    elif seqL < 2000 and seqL >= 1000 :
      model = modDict['1']
      null = nullDict['1']
      print("Model with 1k is used")
    else :
      model = modDict['2']
      null = nullDict['2']
      print("Model with 2k is used")
    ...
    writef = predF.write('\t'.join([head, str(seqL), str(float(score[0,0])), str(float(score[0,1]))])+'\n')
    flushf = predF.flush()
    
    return [head, float(score[0,0]), float(score[0,1])]
...
with open(input_fa, 'r') as faLines :
    ...
    for line in faLines :
        ...
        if flag == 0 and line[0] == '>' :
            print("   predicting: "+str(line))
            # processing line >NC_043042 |Acipenserid herpesvirus 2 strain SRWSHV| partial genome
            head = line.strip()[1:]
            continue
        elif line[0] != '>' :
            seq = seq + line.strip()
            ...
            if countN/len(seq) <= 0.3 and len(seq) >= cutoff_len : 
                ...
                if len(seqname) % 100 == 0 :
                    ...
                    head, score_dna, score_rna = zip(*pool.map(pred, range(0, len(code))))
                    pool.close()
